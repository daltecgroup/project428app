...
class UserFilter {
  ...
  List<User> getFilteredUsers(List<User> users) {
    ...
    List<User> result = users.where((user) {
      ...
      bool matchesRole =
          (showAdmin && user.roles.contains('admin')) ||
          (showFranchisee && user.roles.contains('franchisee')) ||
          (showSpvarea && user.roles.contains('spvarea')) ||
          (showOperator && user.roles.contains('operator')) ||
